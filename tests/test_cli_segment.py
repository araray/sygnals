...
from sygnals.core import segmentation # To mock functions inside
from sygnals.cli import segment_cmd # To mock helpers like _save_segments
...
def mock_audio_read(mocker):
    ...
    # Mock sygnals.core.data_handler.read_data used by the CLI command
...
def mock_segment_save(mocker):
    ...
    # Mock the helper function within the segment_cmd module
    mock = mocker.patch("sygnals.cli.segment_cmd._save_segments")
    return mock

# --- Test Cases for 'segment fixed-length' ---

def test_segment_fixed_length_cmd_success(runner: CliRunner, mock_audio_read, mock_segment_save, tmp_path: Path):
    ...
    # Mock the core segmentation function to check its call args
    mock_core_segment = mocker.patch.object(segmentation, 'segment_fixed_length', return_value=[np.zeros(10), np.zeros(10)]) # Return dummy segments
...
def test_segment_fixed_length_cmd_no_pad(runner: CliRunner, mock_audio_read, mock_segment_save, tmp_path: Path, mocker):
    ...
    input_file.touch()
    output_dir = tmp_path / "output_segments"
    mock_core_segment = mocker.patch.object(segmentation, 'segment_fixed_length', return_value=[np.zeros(10)])
...
def test_segment_fixed_length_cmd_invalid_input(runner: CliRunner, tmp_path: Path, mocker):
    ...
    assert result.exit_code != 0
    assert "Input file" in result.output and "not recognized as audio" in result.output
...
def test_segment_fixed_length_cmd_missing_output(runner: CliRunner, tmp_path: Path):
    ...
    result = runner.invoke(cli, args)
    assert result.exit_code != 0
    assert "Missing option" in result.output and "--output" in result.output
...
def test_segment_fixed_length_cmd_zero_segments(runner: CliRunner, mock_audio_read, tmp_path: Path, mocker):
    ...
    mock_core_segment = mocker.patch.object(segmentation, 'segment_fixed_length', return_value=[])
    input_file = tmp_path / "input.wav"
    input_file.touch()
    output_dir = tmp_path / "output_segments"
